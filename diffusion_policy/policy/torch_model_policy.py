...
class Torch_Model_Policy(BaseImagePolicy):
    def __init__(self,
        ...
        path = pathlib.Path(ViE_path)
        VIE_PARAMS = torch.load(path, pickle_module=dill, map_location=self.device)
        self.VisEncoder.load_state_dict(VIE_PARAMS['state_dicts']['model'])

        logger.info(f"Loading pretrained model from {ViE_path}")
        # Discretizer
        ...
        self.cam_num = len(obs_config['rgb'])
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        image_obs, _ = pack(image_obs, 'bs To * c h w') # bs To N c h w
        
        # 1. obtain image features

        image_fea = self.VisEncoder(
            image_obs,
            ret_loss = False,
